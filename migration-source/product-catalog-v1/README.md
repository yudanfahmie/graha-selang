...
The records are a conservative snapshot of **44 product identities/titles visibly present in the current public Graha Selang product catalog/navigation on 2026-08-09**. This is known current product evidence, not a replacement for the still-required Wave 0 full crawl/reconciliation.
...
It does **not** import or infer price, stock, SKU, technical specification, certification, media, attributes, category membership, brand membership, or commerce settings.
...
The deployable disposable copy is `plugin/graha-selang-site-core/migration-runtime/product-catalog-v1/`. Its manifest and payload are intentionally identical at commit time. After verified target import, only that runtime copy is eligible for cleanup.
