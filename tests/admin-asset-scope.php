...
function add_action() {}
function add_menu_page( $page_title, $menu_title, $capability, $slug, $callback, $icon, $position ) {
	$GLOBALS['menu_calls'][] = func_get_args();
	return 'toplevel_page_' . $slug;
}
function add_submenu_page( $parent, $page_title, $menu_title, $capability, $slug, $callback ) {
	$GLOBALS['submenu_calls'][] = func_get_args();
...
assert_true( 1 === count( $GLOBALS['submenu_calls'] ), 'one Ringkasan submenu registered' );
...
assert_true( 'edit_pages' === $GLOBALS['menu_calls'][0][2], 'overview root capability is explicit' );

$admin->enqueue_assets( 'dashboard_page_other' );
assert_true( 0 === count( $GLOBALS['enqueued_styles'] ), 'unrelated admin screen receives no Graha assets' );

$admin->enqueue_assets( 'toplevel_page_graha-selang-content' );
assert_true( array( 'graha-selang-admin-overview' ) === $GLOBALS['enqueued_styles'], 'Ringkasan screen receives only its stylesheet' );
