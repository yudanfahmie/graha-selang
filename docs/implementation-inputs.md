...
- lifecycle preserves a valid existing static front-page choice and preserves established non-fresh posts-front Reading Settings -- it never forces `show_on_front`/`page_on_front` on an established site;
- presentation ownership of the public front page is decoupled from that Reading Settings choice: the Graha document shell owns whatever WordPress resolves as `is_front_page()` -- latest-posts mode, the canonical static Home, or any other Page an owner has set as front -- so the frontend root is never left to the active theme's own template/"No content found" fallback; a canonical Home Page is useful structural/editor content, not a prerequisite for the shell to render; existing static-front Page content is read, never overwritten;
