...
## 18. Phase D1 accepted implementation contract

Phase D1 implements only the first post-Hero product-storytelling slice of PASS 3 for release `0.7.6`; it does not complete the final 10-section Home and does not start Footer or route-family work.

- `templates/home/content.php` is the actual post-Hero Home composition owner. `templates/native-home.php` remains a narrow compatibility/context bridge, while `TemplateService` keeps native group discovery, real published destination resolution, and front-page shell coordination rather than accumulating new section markup;
- the post-Hero journey is no longer placed inside one shared outer wide container. Each conceptual area owns a full-width `.graha-section` surface with a `.graha-section__inner` plus normal Graha container: buyer orientation uses the default surface, anchor products use the major brand-soft surface, support products use the default surface, and CNG uses a compact controlled-contrast specialist surface. Existing company/proof and closing consultation content remains temporarily as separate full-width bands without being expanded into later-pass sections;
- the six approved Home groups are deliberately split into three zones: Hydraulic Hose / MORGEN and Industrial Hose & Assembly / HAMMER + SUNFLEX are the two anchor showcases; Ducting, PVC Spiral / Spring / Suction, and Fittings / Couplings / Accessories are lower-weight support showcases; CNG / High-pressure Gas Hose is isolated as the specialist composition rather than becoming another support card;
- category artwork is wired only through `AssetService::illustration_url()`: `hydraulic-hose.svg`, `industrial-hose.svg`, `ducting-hose.svg`, `pvc-hose.svg`, `fittings-couplings.svg`, and `cng-hose.svg`. All are normal decorative external images with the established `640 × 480` intrinsic contract, `loading="lazy"`, and asynchronous decoding; the Phase C Hero remains the only eager/high-priority illustration;
- each family shows at most three representative real native `graha_product` links. Home remains discovery, `/products/` remains browsing depth, and product singles remain detail; no synthetic category URL or inferred taxonomy destination is created;
- a product-family card/zone renders only when that Home group contains real published native products. Empty anchor/support groups reduce naturally, an empty CNG group removes the specialist band entirely, and real Product/RFQ/Services/About/Contact destinations continue to be omitted when unavailable rather than replaced with fake links;
- D1 performs presentation reads only: no post/product mutation, taxonomy assignment, migration rerun, new route, invented specification/certification/business fact, external UI dependency, or JavaScript layout system is introduced. D2 remains explicitly out of scope.

## 19. Phase D2 accepted implementation contract

Phase D2 completes the lower Home storytelling journey for release `0.7.7` while preserving the accepted Phase A/B/C/D1 contracts. It does not start Footer/D3 work or add route/content-model behavior.

- `templates/home/content.php` remains the Home composition owner. D2 adds four lower-journey concerns after the D1 family story: a Services-gated technical-capability editorial section, Product/RFQ discovery pathways, a qualitative company/trust layer, and a destination-gated contrast closing section; `templates/native-home.php` remains a narrow bridge and only adds the existing `technical-services.svg` URL through `AssetService::illustration_url()`;
- technical capability renders only when the published Services destination exists. It uses the committed `technical-services.svg` as a decorative external `640 × 480` image with lazy loading/asynchronous decoding and limits copy to the already-approved crimping/assembly, custom fitting/coupling/flange, and product-selection consultation concepts; no certification, workshop-capacity, testing, service-level, coverage, or authorization claim is inferred;
- D2 discovery renders only real Product and specification-need/RFQ pathways. Application/Industry and Brand are omitted because this pass does not establish approved destinations/term discovery for them; no `/applications/`, `/industry/`, `/brands/`, coming-soon card, or synthetic hub is created;
- the trust layer uses qualitative navigation through only available About, Services, Products, RFQ, or Contact destinations. It introduces no invented years, client/product/project counts, geography, partner logo, certification, NAP, or other proof metric;
- the closing journey uses Request Quote as the primary action when available, Contact as the preferred secondary fallback, and Product as the remaining real fallback. It owns no telephone/email/WhatsApp fact or form transport behavior;
- public Home copy is customer-facing Indonesian and no longer exposes implementation vocabulary such as native-product grouping, production surfaces, repository state, or route availability. D1 hierarchy and the Phase C sole-H1/eager-Hero contracts remain intact; all D2 media stays below fold/lazy, and no JavaScript layout, external frontend dependency, route registration, product/taxonomy mutation, migration, or new service owner is introduced.

## 20. Release-candidate finalization contract

This section corrects section 2's now-superseded "Post-hero Home container" description against repository HEAD and records the release-candidate scope implemented for `0.8.0`. Phases A through D2 and Final Visual Polish remain historical and binding; this pass performs no speculative redesign, no new palette/typography/framework/animation system, and preserves the accepted Home/Header/Footer visual system.

- **Home documentation correction.** Section 2 described the post-Hero journey as one shared `graha-container graha-container--wide` wrapper. That has not been true since Phase D1: `TemplateService::render_front_page_shell()` places the Hero and `templates/home/content.php` directly inside `<main>` with no shared outer container, and each Home concern (buyer orientation, anchor products, support products, CNG specialist, technical capability, discovery, trust layer, closing CTA) owns its own full-width `.graha-section` surface. `docs/page-coverage-audit.csv` now reflects this;
- **Search implemented.** `is_search()` now resolves to `templates/search.php`, a document shell that fires `TemplateService::SEARCH_HOOK`. The handler reads the already-executed native `$wp_query` (no second query/index), renders each real matched title/link/excerpt, preserves native `get_the_posts_pagination()`, shows a deliberate empty-result state, and offers real Products/Contact/Request Quote next steps only where those destinations exist. No fake result count, no JavaScript search dependency, and no custom query layer are introduced;
- **404 implemented.** `is_404()` now resolves to `templates/404.php`, firing `TemplateService::NOT_FOUND_HOOK`. `output_not_found_page()` calls the native `status_header( 404 )` before rendering the branded shell with concise Indonesian copy and real Home/Products/Contact/Request Quote links only; it never redirects and never turns an unknown URL into a `200` page;
- **Unknown Page fallback corrected.** `TemplateService::family_for_page_slug()` no longer defaults an unmapped native Page to `legal`. Unmapped Pages now render the neutral `generic_page` family -- a plain branded wrapper that makes no content classification claim. The `legal` family remains declared and fully functional, but is reachable only through an explicit future slug entry in `STATIC_PAGE_FAMILIES`; no such entry is added in this pass, and no legal/privacy/terms Page is created;
- **No route/data expansion.** This pass adds no Application slug, no Articles Hub, no brand discovery invention, and no RFQ/Contact fact. `docs/owner-decision-matrix.md` records the remaining business-input blockers for the owner meeting;
- version synchronized to `0.8.0` across the plugin header, `Kernel::VERSION`, and their guarding tests; `scripts/verify.sh` runs `tests/route-presentation.php` (Search/404 behavior) and `tests/no-dead-links.php` (static scan for `href="#"`/`javascript:` across every plugin-owned template) in addition to the existing suite.
